...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            + 'VERSION:3.0\n' // JAN DI UBAH
            + 'FN: MR ᭄KINGツ XD\n' // NAMA KONTAK OWNER
            + 'ORG:Creator MR ᭄KINGツ;\n' // NAMA CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=+1 (831) 353-5216:+62 822-4658-6611\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '#'
...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["+1 (831) 353-5216@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"+1 (831) 353-5216@s.whatsapp.net"]
			const adminbotnumber = ["+1 (831) 353-5216@s.whatsapp.net"]
			const frendsowner = ["+1 (831) 353-5216@s.whatsapp.net"]
			const premium = ["+1 (831) 353-5216@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *MR ᭄KINGツ✨* 」\n*│+wa.me/+1 (831) 353-5216*╰──────*「 *MR ᭄KINGツ* 」*────*\n\n*_JIKA INGIN MENJADI ADMIN  MR ᭄KINGツ_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					sa = await getBuffer(`https://i.ibb.co/Gv5W97v/IMG-20210106-WA1995.jpg`)
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/+1 (831) 353-5216*╰──────*「 * MR ᭄KINGツ* 」*────*\n\n*_JIKA INGIN MENJADI PREMIUM USER  MR ᭄KINGツ_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
			...
			if (!isPublic) return reply(mess.only.public)
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ * MR ᭄KINGツ Broadcast* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
